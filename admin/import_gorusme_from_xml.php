 ...
 * gorusme_listesi (Görüşmeler sayfası) tablosuna ekler.
...
if (!$tbl || mysqli_num_rows($tbl) === 0) {
    die("gorusme_listesi tablosu veritabanında yok. sql/gorusme_listesi.sql çalıştırın.");
}
...
foreach ($tables as $table) {
    ...
    $sinavTuru = getColumnValue($table, 'SinavTuru');
    if (strtoupper($sinavTuru) === 'NULL' || $sinavTuru === '') {
        $sinavTuru = 'Bursluluk';
    }
    ...
    $sinav_turu_esc = mysqli_real_escape_string($conn, $sinavTuru);

    // gorusme_listesi UNIQUE: (tel_temiz, ogrenci_ad, ogrenci_soyad, sinif, sinav_turu)
    $check = mysqli_query($conn, "SELECT 1 FROM gorusme_listesi WHERE tel_temiz = '$tel_temiz_esc' AND ogrenci_ad = '$ogrenci_ad' AND ogrenci_soyad = '$ogrenci_soyad' AND sinif = $sinif AND IFNULL(sinav_turu,'') = '$sinav_turu_esc' LIMIT 1");
    if ($check && mysqli_num_rows($check) > 0) {
        $atlanan++;
        continue;
    }

    $sql = "INSERT INTO gorusme_listesi (veli_ad, veli_soyad, tel_temiz, tel_orijinal, ogrenci_ad, ogrenci_soyad, sinif, sinav_turu, gorusme_durumu) 
            VALUES ('$veli_ad', '$veli_soyad', '$tel_temiz_esc', '$tel_orijinal', '$ogrenci_ad', '$ogrenci_soyad', $sinif, '$sinav_turu_esc', 'Bekliyor')";
...
@personel_log_ekle($conn, 'import_gorusme_from_xml.php', 'import', [], ['eklenen' => $eklenen, 'atlanan' => $atlanan, 'hata' => $hata]);
header("Location: gorusmeler.php?import_sonuc=" . urlencode($mesaj));
exit;
