...
$erken_kayit_bitis = new DateTimeImmutable('2026-04-19 23:59:59', new DateTimeZone('Europe/Istanbul'));
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Kilitli ve Başarılı Durum Stilleri */
        .s-opts.is-locked { opacity: 0.7; pointer-events: none; }
        .s-opts.is-locked .opt-item { animation: none; border-color: #cbd5e1; }
        .s-opts.is-locked label { cursor: default; color: #64748b; }
        ...
        .s-odeme-basarili i { font-size: 3.5rem; margin-bottom: 14px; color: #22c55e; }
        .s-odeme-basarili h3 { margin: 0 0 10px; font-size: 1.3rem; font-weight: 800; letter-spacing: -0.01em; }
        .s-odeme-basarili p { margin: 0; font-size: 1.05rem; opacity: 0.9; line-height: 1.5; }
...
<?php else: ?>
    ...
    <div class="s-fiyat-row">
        <div class="s-fiyat-left">
            <div class="s-opts <?= $veli_secimi_kilitli ? 'is-locked' : '' ?>">
                <div class="tit">
                    Tercihleriniz &amp; Fırsatlar
                    <?php if ($veli_secimi_kilitli): ?>
                        <span style="float:right; font-size:0.8rem; background:#dcfce7; color:#166534; padding:2px 8px; border-radius:10px;"><i class="fas fa-lock"></i> Kilitli</span>
                    <?php endif; ?>
                </div>
                <label class="opt-item opt-ingilizce opt-lang">
                    <input type="checkbox" id="sec_english" name="ingilizce_ister" value="1" <?= $veli_secimi_kilitli ? 'disabled' : '' ?>>
                ...
                <label class="opt-item opt-almanca opt-lang">
                    <input type="checkbox" id="sec_german" name="almanca_ister" value="1" <?= $veli_secimi_kilitli ? 'disabled' : '' ?>>
                    <span class="opt-lang-text">
        ...
        <div class="s-fiyat-wrap">
            ...
            <div class="fbody">
                ...
                <?php if ($veli_secimi_kilitli): ?>
                <div class="s-odeme-basarili">
                    <i class="fas fa-check-circle"></i>
                    <h3>Ödeme Tamamlandı</h3>
                    <p>Kayıt ödemeniz başarıyla alınmıştır. İşlemlerinizin tamamlanması için danışmanlarımız en kısa sürede sizinle iletişime geçecektir.</p>
                </div>
                <?php else: ?>
                <div class="s-odeme">
                    ...
                    <div class="btns">
                        <form method="post" action="paytr_odeme.php" class="d-inline" id="form-toplu">
                            ...
                            <button type="submit" class="btn-odeme" id="btn-toplu">
                                Ödeme Yap
                            </button>
                        </form>
...
(function() {
    ...
    function getEnglishNormal(early) {
        ...
        return {
            ...
            rows: [
                ...
                early ? { kalem: 'Erken kayıt indirimi (19 Nisan 2026\'ya kadar)', tutar: -earlyInd, indirim: true, bold: false } : null,
    ...
    function hesapla() {
        ...
        var kursOrder = {
            ...
            'Erken kayıt indirimi (19 Nisan 2026\'ya kadar)': 4,
            'Kurs ücreti': 5
        };
        ...
        if (secEnglish && !secGerman) {
            ...
            kitapRows.push({ kalem: 'Almanca Kitap & Materyal ücreti', tutar: MAT_KITAP, bold: false, indirim: false });
            if (erken) {
                kitapRows.push({ kalem: 'İki dilde bir kitap & materyal ücretsiz (19 Nisan 2026\'ya kadar)', tutar: -MAT_KITAP, bold: false, indirim: true });
            } else {
    ...
    if (SINAV_PROFIL === 'onlyEnglish') {
        if (secEnglishEl && !secEnglishEl.disabled) secEnglishEl.checked = true;
        if (secGermanEl && !secGermanEl.disabled) secGermanEl.checked = false;
    } else if (SINAV_PROFIL === 'onlyGerman') {
        if (secEnglishEl && !secEnglishEl.disabled) secEnglishEl.checked = false;
        if (secGermanEl && !secGermanEl.disabled) secGermanEl.checked = true;
    } else {
        if (secEnglishEl && !secEnglishEl.disabled) secEnglishEl.checked = true;
        if (secGermanEl && !secGermanEl.disabled) secGermanEl.checked = true;
    }
